...
interface TransferFiltersProps {
  filters: TransferFilterValues;
  onFiltersChange: (filters: TransferFilterValues) => void;
}
...
export function TransferFilters({ filters, onFiltersChange }: TransferFiltersProps) {
  ...
  const handleFilterChange = (key: keyof TransferFilterValues, value: any) => {
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-80 p-4" align="end">
        <div className="space-y-4">
          ...
          {/* From Account Filter */}
          <div className="space-y-2">
            <Label>{language === "en" ? "Source Account" : "Conta de Origem"}</Label>
            <Select
              value={filters.fromAccountId || "all"}
              onValueChange={(value) => handleFilterChange("fromAccountId", value)}
            >
              <SelectTrigger>
          ...
          {/* To Account Filter */}
          <div className="space-y-2">
            <Label>{language === "en" ? "Destination Account" : "Conta de Destino"}</Label>
            <Select
              value={filters.toAccountId || "all"}
              onValueChange={(value) => handleFilterChange("toAccountId", value)}
            >
              <SelectTrigger>
                <SelectValue placeholder={language === "en" ? "All accounts" : "Todas as contas"} />
              </SelectTrigger>
